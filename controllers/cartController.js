  ...
  try {
    ...
    for (const item of items) {
      ...
      if (isSample && (!quantity || quantity < 1)) {
        return res
          .status(400)
          .json({ message: "Sample must have a valid quantity" });
      }

      if (!isSample && (!floorArea || typeof floorArea !== "object")) {
        return res
          .status(400)
          .json({ message: "Full order must include floor area details" });
      }

      const ProductModel =
        productType === "Wallpaper" ? Wallpaper : WoodenFloor;
      ...
      if (!cart) {
        ...
        const existingItemIndex = cart.items.findIndex(
          (i) => i.productId.toString() === productId && i.isSample === isSample
        );
...
const getGuestCart = async (req, res) => {
  try {
    ...
    const wallpaperItems = items.filter(
      (item) => item.productType === "Wallpaper"
    );
    const woodenFloorItems = items.filter(
...
const removeFromCart = async (req, res) => {
  try {
    ...
    if (!productId || typeof isSample !== "boolean") {
      return res
        .status(400)
        .json({ message: "Product ID and isSample flag are required" });
    }
    ...
    const itemIndex = cart.items.findIndex(
      (item) =>
        item.productId.toString() === productId && item.isSample === isSample
    );
    ...
    // Separate items by their productType
    const wallpaperItems = updatedCart.items.filter(
      (item) => item.productType === "Wallpaper"
    );
    const woodenFloorItems = updatedCart.items.filter(
      (item) => item.productType === "WoodenFloor"
    );
  ...
  try {
    ...
    if (!productId || typeof isSample !== "boolean" || !action) {
      return res.status(400).json({
        message: "Product ID, isSample flag, and action are required",
      });
    }
