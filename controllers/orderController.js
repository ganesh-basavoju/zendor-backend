...
const getOrderDetails = async (req, res) => {
  try {
    ...
    // Separate items by product type for population
    const wallpaperItems = order.items.filter(item => item.productType === 'Wallpaper');
    const woodenFloorItems = order.items.filter(item => item.productType === 'WoodenFloor');
    ...
    const formattedOrder = {
      ...
      items: order.items.map(item => ({
        ...
        name: item.productDetails?.name || item.productName || 'Unknown Product',
        sku: item.productDetails?.sku || `SKU-${item.productId.toString().slice(-6)}`,
        image: item.productDetails?.images?.[0] || item.productDetails?.thumbnail || item.productThumbnail || '/product-placeholder.jpg',
      ...
      timeline: [
        ...
        {
          status: 'Processing',
          date: order.processingAt ? new Date(order.processingAt).toISOString() : null,
          completed: order.status !== 'pending'
        },
        {
          status: 'Shipped',
          date: order.shippedAt ? new Date(order.shippedAt).toISOString() : null,
          completed: ['shipped', 'delivered'].includes(order.status)
        },
        {
          status: 'Delivered',
          date: order.deliveredAt ? new Date(order.deliveredAt).toISOString() : null,
    ...
    const formattedOrder = {
      ...
      timeline: [
        ...
        },
        {
          ...
          completed: ['processing', 'shipped', 'delivered'].includes(updatedOrder.status)
        },
        {
          status: 'Shipped',
...
module.exports = { updateOrderStatus, createOrder, getOrders, updateOrder, getAllOrders, getOrderDetails };
